...
use Modules\Base\Entities\Permission\Permission;
 ...
 * @package Modules\Base\Repositories\Permission
 */
class EloquentPermissionRepository implements PermissionRepository
{
    ...
    public function __construct(
        ...
        PermissionDependencyRepository $dependencies
    )
    {
        $this->roles        = $roles;
    ...
    public function getPermissionsTree($order_by = 'display_name', $sort = 'asc', $withRoles = true)
    {
        if ($withRoles) {
            return Permission::with('roles', 'dependencies.permission')->orderBy($order_by, $sort)->get();
        }
